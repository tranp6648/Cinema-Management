...
import { GetBlogStatus } from "../Services/BlogService";
import { GetCategoryBlog } from "../Services/CategoryBlogService";
function BlogHome() {
    ...
    const filteredGender = Movies.filter(gen =>
        ...
        gen.idCategoryBlog.toString().includes(searchCountry.toLowerCase())
    );
    useEffect(() => {
    ...
    useEffect(() => {
        const fetchdata = async () => {
            try {
                const response = await GetBlogStatus();
                setMovies(response);
                                                ...
                                                <div className="mb-movie-item item-template1">
                                                    <a onClick={() => navigate(`/DetailBlog/${Movies.id}`, { state: { ID: Movies.id, IDAccount: ID } })} style={{ boxShadow: 'none', textDecoration: 'none' }}>
                                                        <div className="movie-image">
                                                            <img src={
                                                                `http://localhost:5277/Images/${Movies.imageUrl}`
                                                            } alt="" />
                                                        </div>
                                                    </a>
                                                    <div className="movie-info">
                                                        <div className="categories-and-time">
                                                            <div className="movie-category">
                                                                <a onClick={() => navigate(`/DetailBlog/${Movies.id}`, { state: { ID: Movies.id, IDAccount: ID } })}>{Movies.categoryBlog.name}</a>
                                                            </div>
